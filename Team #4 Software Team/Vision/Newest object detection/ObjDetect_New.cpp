...
int areaReturn(int currentArea) {
	return currentArea;
}
...
int main() {
	...
	int lowH = 0;//149;//21;							// Set Hue
	int highH = 7;//179;//30;

	int lowS = 50;//110;//200;							// Set Saturation
	int highS = 255;//228;//255;

	int lowV = 145;//185;//102;							// Set Value
	int highV = 255;//255;//225;
	...
	while (charCheckForEscKey != 27 && capWebcam.isOpened()) {				// until the Esc is pressed or webcam connection is lost
		...
		//added 4/26/18
		//this filters out the tiny contours (for the most part) and returns us the biggest contour value
		...
		if (dArea > 10000) {
			...
			if (iLastX >= 0 && iLastY >= 0 && posX >= 0 && posY >= 0) {
				...
				if (posX >= 0 && posX < 50) {
					//hard left
					outputss << "X: " << posX << " Y: " << posY << "| pos 1" << "|Area: " << contourArea(largestContour) << endl;
		...
		*/
		...
		//cout << "Width : " << threshImg.cols << endl;
		//cout << "Height : " << threshImg.rows << endl;
